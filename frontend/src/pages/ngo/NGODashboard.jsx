...
export default function NGODashboard() {
  ...
  // Donations accepted by this NGO but not yet assigned to a volunteer.
  const [needsVolunteer, setNeedsVolunteer] = useState([]);

  // donationId -> recommended volunteers
  const [recommendations, setRecommendations] = useState({});

  const [assigning, setAssigning] = useState(null);

  // Deliveries marked delivered but not yet reviewed by NGO.
  ...
  /**
   * Fetch donations available for this NGO.
   */
  const fetchDonations = () => {
    ...
    api
      .get(`/donations/available?${params}`)
      .then(({ data }) => {
        if (data.success) {
  ...
  /**
   * Fetch donations already accepted by this NGO.
   *
   * This is used for the Accepted statistic and the
   * "accepted donations in progress" notification.
   */
  const fetchAcceptedDonations = () => {
    api
      ...
      .catch(() => {
        // Do not crash the dashboard if this request fails.
        setAcceptedDonations([]);
      });
  };

  /**
   * Fetch deliveries waiting for NGO review.
   */
  const fetchPendingReview = () => {
    setReviewLoading(true);

    api
      .get('/tracking/pending-review')
      .then(({ data }) => {
        if (data.success) {
          setPendingReview(data.logs || []);
        }
      })
      .catch(() => {})
      .finally(() => {
  ...
  const handleAccept = async (id) => {
    ...
    try {
      ...
      if (err.response?.data?.code === 'DONATION_EXPIRED') {
        alert(
          'This donation expired between listing and acceptance and is no longer available. Refreshing the list…'
        );
  ...
  /**
   * Load recommended volunteers for a donation.
   */
  const loadRecommendations = async (donationId) => {
    try {
      const { data } = await api.get(
        `/tracking/recommend/${donationId}`
      );

      if (data.success) {
        setRecommendations((prev) => ({
          ...prev,
          [donationId]: data.recommendations,
        }));
      }
    } catch (err) {
      // NGO can still assign manually if recommendation fails.
    }
  };
  ...
  /**
   * Only pending donations should appear in the
   * available section.
   */
  ...
  /**
   * Accepted donation count.
   *
   * This fixes:
   * ReferenceError: acceptedCount is not defined
   */
  ...
  return (
    <DashboardLayout title="NGO Dashboard">
      ...
      {/* Delivery ready for review notification */}
      <AnimatePresence>
        {reviewNotification && (
          <motion.div
            initial={{
          ...
          >
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-blue-700 mt-0.5">
                {reviewNotification.volunteerName}{' '}
                delivered{' '}
                {reviewNotification.foodName}.
                Confirm receipt and rate them.
              </div>
            </div>

            <Link
              to={`/volunteer/track/${reviewNotification.donationId}`}
            >
            ...
            <button
              ...
              className="text-blue-400 hover:text-blue-600 text-xs font-medium ml-1"
            >
              ✕
            </button>
          </motion.div>
        ...
        >
          <div>
            <div className="text-sm font-semibold text-amber-900">
              Set your organization location to see nearby donations
            </div>

            <div className="text-xs text-amber-700 mt-0.5">
              We use it to match and rank donations by distance.
            </div>
          </div>
        ...
        pendingReview.length > 0 && (
          <div className="mb-6">
            ...
            <SectionHeader
              ...
              sub={`${pendingReview.length} delivered — confirm receipt & rate`}
            />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              ...
              {pendingReview.map((log) => (
                ...
                >
                  ...
                  <Link
                    to={`/volunteer/track/${log.donation?._id}`}
                  >
      ...
      {needsVolunteer.length > 0 && (
        <div className="mb-6">
          ...
          <SectionHeader
            ...
            sub="Accepted donations awaiting pickup assignment"
          />
              ...
              >
                ...
                {!recommendations[d._id] ? (
                  ...
                  <p className="text-xs text-gray-400">
                    No verified volunteers available nearby right now.
                  </p>
                            ...
                            <div className="flex flex-wrap gap-1 mb-2">
                              ...
                              {v.badges
                                ...
                                .map((b) => (
                                  ...
                                  <span
                                    key={b}
                                    className="text-xs px-1.5 py-0.5 rounded-full bg-white text-gray-500 border border-gray-100"
                                  >
                                    🏅 {b}
                                  </span>
      ...
      {/* Filters */}
      <div
        className="bg-white rounded-2xl p-4 mb-6 flex flex-wrap gap-3 items-center"
        style={{
          border:
            '1.5px solid #f0fdf4',
          boxShadow:
      ...
      >
        ...
        <div className="relative flex-1 min-w-48">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:border-green-400 focus:outline-none"
          />
        ...
        <motion.button
          ...
          className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-green-600 text-white text-sm font-medium"
        >
          <FiRefreshCw size={15} />
          Apply
        </motion.button>
        ...
        >
          ...
          <div>
            ...
            <div className="text-xs text-green-700 mt-0.5">
              ...
              track deliveries, from Accepted
              Donations.
            </div>
      ...
      <div>
        ...
        {loading ? (
          ...
          <EmptyState
            icon="🍽️"
            message="No donations available in your area right now. Check back soon!"
          />
